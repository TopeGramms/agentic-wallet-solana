...
exports.runWalletAgent = runWalletAgent;
const sdk_1 = __importDefault(require("@anthropic/sdk"));
...
async function executeTool(toolName, toolInput, wallet) {
    try {
        switch (toolName) {
            case "get_balance": {
            ...
            case "send_sol": {
                const sig = await wallet.sendSOL(toolInput.to_address, toolInput.amount);
            ...
            case "get_spl_token_balance": {
                const balance = await wallet.getSPLTokenBalance(toolInput.mint_address);
                return `SPL Token balance: ${balance}`;
            }
            case "send_spl_token": {
                const sig = await wallet.sendSPLToken(toolInput.mint_address, toolInput.to_address, toolInput.amount);
...
// ─── Main AI Agent loop ────────────────────────────────────────────────────
async function runWalletAgent(userInstruction, wallet) {
        ...
        if (response.stop_reason === "tool_use") {
            ...
            for (const block of response.content) {
                if (block.type === "tool_use") {
                    ...
                    const result = await executeTool(block.name, block.input, wallet);
